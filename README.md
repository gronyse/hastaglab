...
Instagram 해시태그는 최종적으로 5개로 제한합니다. TikTok과 Naver Blog는 8~15개를 생성합니다.
태그 정리, 중복 제거, 입력에 없는 구체 지역명 제거는 백엔드에서 처리합니다.
...
`REDIS_URL`을 설정하면 제한 기록은 Redis에 저장됩니다. 설정하지 않으면 서버 메모리 방식으로 동작합니다.
